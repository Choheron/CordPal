...
def getAlbumOfDay(request: HttpRequest, date: str = ""):
  ...
  if(request.method != "GET"):
    logger.warning(f"getAlbumOfDay called with a non-GET method, returning 405.", extra={'crid': request.crid})
  ...
  out['date'] = date
  logger.info(f"Returning Album of Day Object for Date {date}...", extra={'crid': request.crid})
  return JsonResponse(out)
...
def setAlbumOfDay(request: HttpRequest):
  ...
  if(request.method != "POST"):
    logger.warning(f"setAlbumOfDay called with a non-POST method, returning 405.", extra={'crid': request.crid})
    res = HttpResponse("Method not allowed")
    res.status_code = 405
    return res
  ...
  try:
    ...
    logger.warning(f"WARN: Album of the day already selected: {currDayAlbum}", extra={'crid': request.crid})
  ...
  while(not selected):
    ...
    if(len(albumPool) == 0):
      logger.error(f"WARNING! NO ELIGIBLE ALBUMS FOR SELECTION! NO ALBUM WILL BE SELECTED", extra={'crid': request.crid})
...
def setAlbumOfDayADMIN(request: HttpRequest, date: str, mbid: str):
  # Make sure request is a post request
  if(request.method != "POST"):
    logger.warning(f"setAlbumOfDayADMIN called with a non-POST method, returning 405.", extra={'crid': request.crid})
    res = HttpResponse("Method not allowed")
    res.status_code = 405
    return res
  # Get request body
...
def getAotdDates(request: HttpRequest, mbid: str):
  # Make sure request is a get request
  if(request.method != "GET"):
    logger.warning(f"getAotdDates called with a non-GET method, returning 405.", extra={'crid': request.crid})
    res = HttpResponse("Method not allowed")
    res.status_code = 405
    return res
...
def calculateAOTDChances(request: HttpRequest):
  # Make sure request is a post request
  if(request.method != "POST"):
    logger.warning(f"calculateAOTDChances called with a non-POST method, returning 405.", extra={'crid': request.crid})
    res = HttpResponse("Method not allowed")
    res.status_code = 405
    return res
  # Get current date
  ...
  for aotdUser in aotd_users:
    ...
    if(user.pk in user_outage_map):
      logger.info(f"\t{aotdUser.user.nickname} is under an outage!", extra={'crid': request.crid})
      outage = UserAlbumOutage.objects.filter(start_date__lte=tomorrow, end_date__gte=tomorrow).get(user=user)
      logger.info(f"User {user.nickname} is currently under an outage, lasts until {outage.end_date.strftime('%Y-%m-%d')}!", extra={'crid': request.crid})
...
def getAOtDByMonth(request: HttpRequest, year: str, month: str):
  # Make sure request is a get request
  if(request.method != "GET"):
    logger.warning(f"getAOtDByMonth called with a non-GET method, returning 405.", extra={'crid': request.crid})
    res = HttpResponse("Method not allowed")
    res.status_code = 405
    return res
...
def getDayTimelineData(request: HttpRequest, aotd_date: str):
  # Make sure request is a get request
  if(request.method != "GET"):
    logger.warning(f"getDayTimelineData called with a non-GET method, returning 405.", extra={'crid': request.crid})
    res = HttpResponse("Method not allowed")
    res.status_code = 405
    return res
  # Get aotd object
  try:
    ...
    logger.error(f"AOTD Object not found for passed in date: {aotd_date}", extra={'crid': request.crid})
