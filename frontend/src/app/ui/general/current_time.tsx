      ...
      <div className="text-6xl min-w-[400px] px-5">
        <div className="flex w-full py-3">
          <div className="flex w-full">
            <p className="w-1/3 text-center">{padNumber(time.getHours())}</p>
            <p>:</p>
            <p className="w-1/3 text-center">{padNumber(time.getMinutes())}</p>
            <p>:</p>
            <p className="w-1/3 text-center">{padNumber(time.getSeconds())}</p>
          </div>
          <p>{time.toLocaleTimeString().substring(time.toLocaleTimeString().length-2)}</p>
        </div>
      </div>
