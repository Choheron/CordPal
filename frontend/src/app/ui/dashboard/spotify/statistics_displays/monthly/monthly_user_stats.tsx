...
export default function MonthlyUserStats(props) {
  ...
  const displayAlbums = (discord_id) => {
    if(getUserSubData(discord_id, 'submissions') == 0) {
      return <></>
    }
    return getUserSubData(discord_id, 'submissions').map((album, index) => {
      return (
        <div 
          className="group relative flex h-[115px] w-[115px] m-1"
          key={index}
        >
          <img 
            src={album['cover_url']}
  ...
  return (
    <div className="w-fit mx-auto border bg-zinc-800/30 border-neutral-800 rounded-2xl py-2">
      <Tabs
        isVertical={true}
        items={users}
        className="ml-2"
      >
        {(item: any) => (
          ...
          >
            <div className="h-full w-full flex flex-col backdrop-blur-2xl px-2 py-2 rounded-2xl bg-zinc-800/30 border border-neutral-800">
              <p className="font-extralight w-full text-center text-xl mb-1">
                {monthName} {year} Statistics for {item.nickname}:
              </p>
              <Divider />
              <div className="flex flex-col lg:flex-row gap-2 font-extralight">
                {/* Review Stats for User */}
                <div className="flex flex-col mt-2 text-center border border-zinc-800 rounded-xl p-2 px-3 bg-slate-400/10">
                ...
                <div className="flex flex-col mt-2 text-center border border-zinc-800 rounded-xl p-2 px-3 bg-slate-400/10">
                  <p className="text-xl">Submission/Selection Statistics:</p>
                  <Divider />
                  <div className="flex justify-between w-full mt-1">
                  ...
                  <div className="w-full h-full rounded-2xl bg-black/30 border border-neutral-800 -pb-10">
                    ...
                    <div className="relative h-[92%] bg-black/90 rounded-b-2xl">
                      <div className="absolute flex flex-wrap justify-between w-full max-h-full overflow-auto scrollbar-hide mx-2 pr-4">
                        {displayAlbums(item.discord_id)}
                      </div>
                    </div>
                  </div>
                </div>
